...
function error(error, req, res, _next) {
  ...
  // ✅ Always return JSON for frontend (no HTML rendering)
...
function verify(req, res, next) {
  ...
  errors.forEach((e) => {
    if (!formattedErrors[e.param]) formattedErrors[e.param] = e.msg;
  });

  throw new CustomError(error, 400);
}

// ✅ Query parser (pagination, search, limit)
...
// ✅ Rate limiting middleware (uses Redis if enabled)
...
// ✅ Admin setup — used only during first-time startup
async function adminSetup(req, res, next) {
  ...
  // Instead of redirect (HTML), send JSON message
  return res.status(200).json({
    setupRequired: true,
    message: "No users found. Please create the initial admin user.",
  });
}
